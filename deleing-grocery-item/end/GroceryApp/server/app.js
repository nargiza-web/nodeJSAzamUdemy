...
app.delete('/api/grocery-items/:groceryItemId', async (req,res) => {

  const groceryItemId = req.params.groceryItemId

  ShoppingList.findOne({'groceryItems._id': groceryItemId},(error, shoppingList) => {
    if(error) {
      res.json({message: 'Unable to delete grocery item!'})
    } else {
      shoppingList.groceryItems = shoppingList.groceryItems.filter((gi) => gi._id != groceryItemId)
